<div class="row">
    <div class="form-group col-md-3 @if ($errors->has('prf_mod_id')) has-error @endif">
        <label for="prf_mod_id" class="form-label">Módulo*</label>
        <div class="controls">
            <select name="prf_mod_id" class="form-control" id="mod_id">
                <option value="">Selecione um módulo</option>
                @foreach($modulos as $key => $value)
                    <option value="{{ $key }}" {{ old('prf_mod_id', $perfil->prf_mod_id ?? '') == $key ? 'selected' : '' }}>{{ $value }}</option>
                @endforeach
            </select>
    ...
    <div class="form-group col-md-9 @if ($errors->has('prf_nome')) has-error @endif">
        <label for="prf_nome" class="form-label">Nome do perfil*</label>
        <div class="controls">
            <input type="text" name="prf_nome" value="{{ old('prf_nome', $perfil->prf_nome ?? '') }}" class="form-control" >
...
<div class="form-group @if ($errors->has('prf_descricao')) has-error @endif">
    <label for="prf_descricao" class="form-label">Descrição do perfil</label>
    <div class="controls">
        <input type="text" name="prf_descricao" value="{{ old('prf_descricao', $perfil->prf_descricao ?? '') }}" class="form-control" >
