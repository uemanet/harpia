...
{{--                <img src="{{url('/')}}/img/logo.png" style="height:70px" />--}}
                <h1>Matriculas</h1>
                <h4 class="text-center" style="margin-top:2px">Sistema de <b>Matrícula Online</b></h4>
            </div>
            <div class="box-content">
                <div class="login-box-body">
                    @if (count($errors) > 0)
                        <div class="alert alert-danger">
                            <strong>Whoops! </strong>E-mail e/ou CPF incorreto(s).
                        </div>
                    @endif
                    <p class="login-box-msg">Informe os dados utilizados no sistema de seletivos</p>
                    <form action="{{url('/matriculas-alunos/login')}}" method="post">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <div class="form-group has-feedback @if ($errors->has('usr_usuario')) has-error @endif">
                            {!! Form::text('usr_usuario', old('usr_usuario'), array('placeholder' => 'Informe seu e-mail', 'class'=>'form-control')) !!}
                        ...
                        <div class="form-group has-feedback @if ($errors->has('usr_senha')) has-error @endif">
                            {!! Form::text('usr_senha', old('usr_senha'), array('placeholder' => 'Informe seu CPF', 'class'=>'form-control')) !!}
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                <button type="submit" class="btn btn-primary btn-block btn-flat mt-lg">Validar Informações</button>
                            </div><!-- /.col -->
                        </div>
                    </form>
                </div>
