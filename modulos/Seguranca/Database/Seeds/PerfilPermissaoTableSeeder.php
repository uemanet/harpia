...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([127, 128, 129, 130, 131, 132, 133]); // Permissoes do recurso Ambientes

        /** Perfil Administrador do Módulo de Monitoramento */

        $perfil = Perfil::find(5);

        $perfil->permissoes()->attach([134]); // Permissoes do recurso Dashboard

        $perfil->permissoes()->attach([135, 136]); // Permissoes do recurso Tempo Online
    }
}
