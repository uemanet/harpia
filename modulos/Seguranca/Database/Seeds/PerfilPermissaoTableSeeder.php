...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([23, 24, 25, 26, 27]); // Permissoes do recurso Usuários


        /** Perfil Administrador do Módulo Geral */
        $perfil = Perfil::find(2);

        $perfil->permissoes()->attach([28]); // Permissões Dashboard

        $perfil->permissoes()->attach([29, 30, 31, 32, 33]); //Permissões Pessoas

        /** Perfil Administrador do Módulo Acadêmico */

        $perfil = Perfil::find(3);

        $perfil->permissoes()->attach([34]); //Index Dashboard

        $perfil->permissoes()->attach([35, 36, 37, 38]); //Permissões do recurso polo

        $perfil->permissoes()->attach([39, 40, 41, 42]); // Permissoes do recurso departamentos

        $perfil->permissoes()->attach([43, 44, 45, 46]); // Permissoes do recurso periodos letivos

        $perfil->permissoes()->attach([47, 48, 49, 50]); // Permissoes do recurso cursos

        $perfil->permissoes()->attach([51, 52, 53, 54]); // Permissoes do recurso centros

        $perfil->permissoes()->attach([55, 56, 57, 58]); // Permissoes do recurso matrizes curriculares
