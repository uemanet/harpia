...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([128, 129]); // Permissoes do recurso Historico Parcial
        ...
        $perfil->permissoes()->attach([131, 132, 133, 134, 135, 136, 137]); // Permissoes do recurso Ambientes

        /** Perfil Administrador do Módulo de Monitoramento */

        $perfil = Perfil::find(5);

        $perfil->permissoes()->attach([138]); // Permissoes do recurso Dashboard

        $perfil->permissoes()->attach([139, 140]); // Permissoes do recurso Tempo Online
    }
}
