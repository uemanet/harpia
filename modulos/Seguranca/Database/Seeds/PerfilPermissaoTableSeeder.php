...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([126]); // Permissoes do recurso Relatórios

        $perfil->permissoes()->attach([127, 128]); // Permissoes do recurso Relatórios Matriculas

        $perfil->permissoes()->attach([129, 130, 131]); // Permissoes do recurso Historico Parcial
        ...
        $perfil->permissoes()->attach([133, 134, 135, 136, 137, 138, 139]); // Permissoes do recurso Ambientes

        /** Perfil Administrador do Módulo de Monitoramento */

        $perfil = Perfil::find(5);

        $perfil->permissoes()->attach([140]); // Permissoes do recurso Dashboard

        $perfil->permissoes()->attach([141, 142]); // Permissoes do recurso Tempo Online
    }
}
