...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([66, 67, 68, 69, 70]); // Permissoes do recurso módulos matrizes

        $perfil->permissoes()->attach([71, 72, 73, 74]); // Permissoes do recurso vinculos

        $perfil->permissoes()->attach([75, 76, 77]); // Permissoes do recurso tutores do grupo

        $perfil->permissoes()->attach([78, 79, 80, 81]); // Permissoes do recurso disciplinas
    }
}
