...
class PerfilPermissaoTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $perfil->permissoes()->attach([89, 90, 91, 92]); // Permissoes do recurso Matricular Aluno no Curso

        $perfil->permissoes()->attach([93, 94]); // Permissoes do recurso Ofertar Disciplina

        $perfil->permissoes()->attach([95, 96]); // Permissoes do recurso Matricular Aluno na Disciplina

        $perfil->permissoes()->attach([97]); // Permissoes do recurso Matriculas em Lote

        $perfil->permissoes()->attach([98, 99, 100, 101, 102]); // Permissoes do recurso Lançamento de TCC
        ...
        $perfil->permissoes()->attach([129, 130]); // Permissoes do recurso histórico definitivo
        ...
        $perfil->permissoes()->attach([135]); // Permissoes do recurso certificação

        $perfil->permissoes()->attach([136]); // Permissoes do recurso controle de registro

        /** Perfil Administrador do Módulo Integração */

        $perfil = Perfil::find(4);

        $perfil->permissoes()->attach([137]); // Permissoes do recurso Dashboard

        $perfil->permissoes()->attach([138, 139, 140, 141, 142, 143, 144]); // Permissoes do recurso Ambientes

        /** Perfil Administrador do Módulo de Monitoramento */

        $perfil = Perfil::find(5);

        $perfil->permissoes()->attach([145]); // Permissoes do recurso Dashboard

        $perfil->permissoes()->attach([146, 147]); // Permissoes do recurso Tempo Online
    }
}
