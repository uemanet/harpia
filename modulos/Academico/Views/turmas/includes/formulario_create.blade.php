<div class="row">
    ...
    @endforeach
              ...
              @else <select name="trm_per_id" class="form-control">
    <option value="">Sem Períodos Letivos Disponíveis</option>
    @foreach($periodosletivos as $key => $value)
        <option value="{{ $key }}" {{ old('trm_per_id') == $key ? 'selected' : '' }}>{{ $value }}</option>
    @endforeach
</select>@endif
...
<div class="row">
    ...
    <div class="form-group col-md-2 @if ($errors->has('trm_tipo_integracao')) has-error @endif">
        <label for="trm_tipo_integracao" class="form-label">Tipo de integração <small class="obrigatorio-dot">*</small></label>
        <div class="controls">
            <select name="trm_tipo_integracao" class="form-control">
    <option value="">Selecione</option>
    @foreach(array('v1' => 'Versão 1', 'v2' => 'Versão 2') as $key => $value)
        <option value="{{ $key }}">{{ $value }}</option>
    @endforeach
</select>
