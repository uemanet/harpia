    ...
    public function up()
    {
        Schema::table('acd_matriculas', function (Blueprint $table) {
            DB::statement("ALTER TABLE acd_matriculas CHANGE COLUMN mat_modo_entrada mat_modo_entrada ENUM('vestibular', 'transferencia_externa',
                          'transferencia_interna_de', 'transferencia_interna_para','outros_tipos_selecao','outras_formas_ingresso', 'transferencia_obrigatoria', 'transferencia_ex_oficio', 
                          'graduando_interno', 'graduando_externo', 'graduado')");
        });
    }
